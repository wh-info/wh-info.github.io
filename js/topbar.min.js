...
function resetToDefaults() {
    topbar.config({
      ...
      shadowBlur   : 10,
      shadowColor  : 'rgba(103,   187,   235,   .6)',
      className    : 'topbar',
    })
  }
  ...
  // attributes bright hover
const triggers = document.querySelectorAll(
  "#blankC, #brightRepawn, #brightSpawn, #brightLeadsTo, #brightindMassB, #brightindMass, #brightToMassB, #brightToMass, #brightlifeTimeB, #brightlifeTime, #brightSigLevelB, #brightSigLevel"
  );

triggers.forEach((trigger) => {
  trigger.addEventListener("mouseenter", () => {
    document.querySelector(".attri").style.color = "#87c2e4ff";
  });

  trigger.addEventListener("mouseleave", () => {
    document.querySelector(".attri").style.color = "#287099";
  });
});

 // wormholes bright hover
const rows = document.querySelectorAll("#rowOne, #rowTwo, #rowThree");

rows.forEach((row) => {
  row.addEventListener("mouseenter", () => {
    document.querySelector(".wholes").style.color = "#87c2e4ff";
  });

  row.addEventListener("mouseleave", () => {
    document.querySelector(".wholes").style.color = "#287099";
  });
});

 // whtypes headeropacity hover 
const opacityTriggers = document.querySelectorAll(
  "#blankC, #rowOne, #rowTwo, #rowThree, " +
  "#brightRepawn, #brightSpawn, #brightLeadsTo, " +
  "#brightindMassB, #brightindMass, " +
  "#brightToMassB, #brightToMass, " +
  "#brightlifeTimeB, #brightlifeTime, " +
  "#brightSigLevelB, #brightSigLevel"
);

const something = document.querySelector("#whtypeH");

opacityTriggers.forEach((el) => {
  el.addEventListener("mouseenter", () => {
    something.style.opacity = "0.88"; // fade
  });

  el.addEventListener("mouseleave", () => {
    something.style.opacity = "1"; // restore
  });
});